...
8. **Tamper-evident record**
   ...
   - Release certificates carry an **Ed25519 public-key signature** over their own contents. The public key is printed on the certificate by fingerprint, served from `/api/audit/certificate-key`, and carried inline in the certificate's JSON with the exact signed bytes — so a receiving railway or an auditor verifies it themselves, without this server and without being able to issue one. **`/verify.html`** does that in any browser with no request to anything.
   - **A wagon passport**: the wagon's whole record here — every stage timestamp, spring height, verdict, part off and on, photograph hash, the sign-off and the certificate's own signature — as one file, hash-linked event by event and sealed by the same key, for the wagon to carry to its next overhaul. The next shop imports it; it is verified against the key inside it, kept whole, never merged, and shown beside that shop's own record. Federation without a central server.
...
- **The camera cannot be better than the people who labelled its examples.** Its condition heads (surface, damage) judge from a photograph, and may not claim 95% until the shop's own blind reads — one inspector judging another's photograph without the label — show a person can judge condition from a picture at 95% on thirty reads. Until then a head is held at "flag only" whatever its own score says, and the bench says why. Wagon parts have no blind read yet; that is a stated gap.
...
- **Testing**: 953 server tests, 266 client tests, and 39 root-harness E2E suites (7 of which drive the real server; the rest test a parallel mock harness) (Boundary Value Analysis, Combinatorial, High-Load multi-shift simulation, and adversarial integrity tests).
...
The suite that exercises the real server is `server/tests` — **953 tests** in 97 files. Run it with:
