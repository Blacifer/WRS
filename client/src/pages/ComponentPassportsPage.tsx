...
import { AlertTriangleIcon, CameraIcon, FileTextIcon, GearIcon, IdCardIcon, TrainIcon } from '../components/Icons.tsx';
import type {
  SerializedComponent,
...
export const ComponentPassportsPage: React.FC<ComponentPassportsPageProps> = ({ onNavigateToWagon }) => {
  ...
  const getHealthBadge = (healthStatus: ComponentHealthStatus, score: number) => {
    switch (healthStatus) {
      case 'EXCELLENT':
        return (
          <span className="px-2.5 py-0.5 rounded-full text-xs font-extrabold bg-good-soft text-good-ink border border-good-line flex items-center gap-1.5 shadow-sm">
      ...
      case 'GOOD':
        return (
          <span className="px-2.5 py-0.5 rounded-full text-xs font-extrabold bg-accent-soft text-accent-ink border border-accent-line flex items-center gap-1.5 shadow-sm">
      ...
      case 'FAIR':
        return (
          <span className="px-2.5 py-0.5 rounded-full text-xs font-extrabold bg-warn-soft text-warn-ink border border-warn-line flex items-center gap-1.5 shadow-sm">
      ...
      default:
        return (
          <span className="px-2.5 py-0.5 rounded-full text-xs font-extrabold bg-bad-soft text-bad-ink border border-bad-line flex items-center gap-1.5 shadow-sm">
  ...
  const getStatusBadge = (status: ComponentStatus) => {
    switch (status) {
      case 'IN_SERVICE':
        return <span className="px-2 py-0.5 text-[11px] font-bold rounded bg-good-soft text-good-ink border border-good-line">{isHi ? 'सेवा में' : 'IN SERVICE'}</span>;
      case 'AVAILABLE_IN_STORES':
        return <span className="px-2 py-0.5 text-[11px] font-bold rounded bg-accent-soft text-accent-ink border border-accent-line">{isHi ? 'स्टोर्स डिपो' : 'STORES DEPOT'}</span>;
      case 'UNDER_MAINTENANCE':
        return <span className="px-2 py-0.5 text-[11px] font-bold rounded bg-warn-soft text-warn-ink border border-warn-line">{isHi ? 'अनुरक्षण' : 'MAINTENANCE'}</span>;
      case 'RECONDITIONED':
        return <span className="px-2 py-0.5 text-[11px] font-bold rounded bg-accent-soft text-accent-ink border border-accent-line">{isHi ? 'पुनर्निर्मित' : 'RECONDITIONED'}</span>;
      case 'CONDEMNED':
        return <span className="px-2 py-0.5 text-[11px] font-bold rounded bg-bad-soft text-bad-ink border border-bad-line">{isHi ? 'कंडम' : 'CONDEMNED'}</span>;
      default:
        return <span className="px-2 py-0.5 text-[11px] font-bold rounded bg-raised text-ink-body">{status}</span>;
    }
  };
  ...
  return (
    <div className="space-y-6">
      {/* Top Banner & Actions */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 p-5 bg-card border border-accent-line rounded-card">
        <div className="flex items-center gap-3.5">
          <div className="w-12 h-12 rounded-control bg-accent-soft border border-accent-line flex items-center justify-center text-accent-ink text-2xl font-extrabold">
            <IdCardIcon size={20} />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl sm:text-2xl font-extrabold text-white tracking-tight">{isHi ? 'घटक हेल्थ पासपोर्ट व क्रमांकन' : 'Component Health Passports & Serialization'}</h1>
              <span className="px-2 py-0.5 text-[10px] font-extrabold uppercase bg-accent-soft text-accent-ink border border-accent-line rounded">
                RDSO G-95 Rev-II
              </span>
            </div>
        ...
        <div className="flex items-center gap-2.5 w-full md:w-auto">
          <button
            ...
            className="flex-1 md:flex-none min-h-[44px] px-4 py-2 bg-accent hover:bg-accent-hover text-white font-bold text-xs rounded-control flex items-center justify-center gap-2 transition active:scale-95 border border-accent-line"
          >
            <CameraIcon size={16} />
          ...
          <button
            ...
            className="flex-1 md:flex-none min-h-[44px] px-4 py-2 bg-raised hover:bg-selected text-white font-bold text-xs rounded-control border border-line flex items-center justify-center gap-1.5 transition active:scale-95"
          >
            <span>+</span>
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
          <div className="p-3.5 bg-card border border-line rounded-control">
            <p className="text-[11px] text-ink-muted font-semibold uppercase tracking-[0.07em]">{isHi ? 'कुल ट्रैक किए गए' : 'Total Tracked'}</p>
            <p className="text-xl font-extrabold text-white mt-1">{stats.totalComponents}</p>
            <p className="text-[10px] text-accent-ink mt-0.5">{isHi ? 'क्रमांकित इकाइयाँ' : 'Serialized Units'}</p>
          </div>

          <div className="p-3.5 bg-card border border-line rounded-control">
            <p className="text-[11px] text-ink-muted font-semibold uppercase tracking-[0.07em]">{isHi ? 'सेवा में' : 'In Service'}</p>
            <p className="text-xl font-extrabold text-good-ink mt-1">{stats.inService}</p>
            <p className="text-[10px] text-good-ink/80 mt-0.5">{isHi ? 'वैगनों पर लगे' : 'Mounted on Wagons'}</p>
          </div>

          <div className="p-3.5 bg-card border border-line rounded-control">
            <p className="text-[11px] text-ink-muted font-semibold uppercase tracking-[0.07em]">{isHi ? 'स्टोर्स डिपो' : 'Stores Depot'}</p>
            <p className="text-xl font-extrabold text-accent-ink mt-1">{stats.availableInStores}</p>
            <p className="text-[10px] text-accent-ink mt-0.5">{isHi ? 'जारी करने हेतु तैयार' : 'Ready for Issue'}</p>
          </div>

          <div className="p-3.5 bg-card border border-line rounded-control">
            <p className="text-[11px] text-ink-muted font-semibold uppercase tracking-[0.07em]">{isHi ? 'अनुरक्षण' : 'Maintenance'}</p>
            <p className="text-xl font-extrabold text-warn-ink mt-1">{stats.underMaintenance}</p>
            <p className="text-[10px] text-warn-ink/80 mt-0.5">{isHi ? 'कारखाना बे में' : 'In Workshop Bay'}</p>
          </div>

          <div className="p-3.5 bg-card border border-line rounded-control">
            <p className="text-[11px] text-ink-muted font-semibold uppercase tracking-[0.07em]">{isHi ? 'पुनर्निर्मित' : 'Reconditioned'}</p>
            <p className="text-xl font-extrabold text-accent-ink mt-1">{stats.reconditioned}</p>
            <p className="text-[10px] text-accent-ink mt-0.5">{isHi ? 'पीओएच ओवरहॉल किए' : 'POH Overhauled'}</p>
          </div>
          ...
          <div className="p-3.5 bg-card border border-line rounded-control">
            <p className="text-[11px] text-ink-muted font-semibold uppercase tracking-[0.07em]">{isHi ? 'गंभीर चेतावनी' : 'Critical Alert'}</p>
            <p className="text-xl font-extrabold text-bad-ink mt-1">{stats.criticalHealthCount}</p>
            <p className="text-[10px] text-bad-ink/80 mt-0.5">{isHi ? 'ध्यान आवश्यक' : 'Needs Attention'}</p>
          </div>
        </div>
      )}

      {/* Filter & Search Bar */}
      <div className="p-4 bg-card border border-line rounded-control space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          ...
          <div className="lg:col-span-2">
            <input
              ...
              className="w-full px-3.5 py-2 bg-page border border-line rounded-control text-xs text-white placeholder-slate-500 focus:outline-none focus:border-accent-line font-mono"
            />
          </div>
          ...
          <div>
            <select
              ...
              className="w-full px-3 py-2 bg-page border border-line rounded-control text-xs text-white focus:outline-none focus:border-accent-line"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className={`space-y-3 ${selectedComponent ? 'lg:col-span-2' : 'lg:col-span-3'}`}>
          {loading ? (
            <div className="p-12 text-center text-ink-muted bg-card rounded-control border border-line">
            ...
            <div className="p-4 bg-bad-soft border border-bad-line rounded-control text-bad-ink text-xs">
              <AlertTriangleIcon size={15} className="inline align-[-2px] mr-1.5" />{error}
            </div>
          ) : components.length === 0 ? (
            <div className="p-12 text-center text-ink-muted bg-card rounded-control border border-line">
              <p className="text-base font-bold text-ink-body">{isHi ? 'कोई क्रमांकित घटक नहीं मिला' : 'No Serialized Components Found'}</p>
              <p className="text-xs text-ink-faint mt-1">Try adjusting your filters or register a new component.</p>
            </div>
          ) : (
            <div className="overflow-x-auto bg-card border border-line rounded-control">
                    ...
                    return (
                      <tr
                        ...
                        className={`cursor-pointer transition-colors ${
                          isSelected
                            ? 'bg-accent-soft border-l-4 border-l-cyan-500'
                            : 'hover:bg-raised'
                        }`}
                      >
                        {/* Serial & Type */}
                        <td className="p-3 sm:p-4">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] px-1.5 py-0.5 rounded bg-raised text-ink-body border border-line">
                              QR
                            </span>
                          </div>
                          <p className="text-[11px] text-ink-body font-medium truncate max-w-[200px]">
                            {comp.partName}
                          </p>
                        </td>
                          ...
                          {comp.currentWagonNumber ? (
                            <div>
                              <button
                                ...
                                className="text-accent-ink hover:text-accent-ink font-mono font-bold hover:underline flex items-center gap-1"
                              >
                                <TrainIcon size={14} />
                            ...
                            <div>
                              <span className="text-ink-muted font-medium">{isHi ? 'डिपो स्टोर्स' : 'Depot Stores'}</span>
                              <span className="text-[10px] text-accent-ink block font-mono">
                        ...
                        {/* Mfg / Overhauls */}
                        <td className="p-3 sm:p-4 text-ink-muted">
                          ...
                          */}
                          {comp.componentType === 'BEARING' && (
                            <p className="text-[10px] text-warn-ink/90 font-semibold">
                        ...
                        <td className="p-3 sm:p-4 text-right">
                          <div className="flex items-center justify-end gap-1.5" onClick={(e) => e.stopPropagation()}>
                            ...
                            <button
                              ...
                              className="px-2.5 py-1 bg-accent-soft hover:bg-accent-hover text-white rounded text-[11px] font-bold transition shadow"
                            >
                              POH
                            </button>
                            {comp.componentType === 'BEARING' && ((comp as any).rohCyclesSincePoh ?? 0) < 3 && (
                              <button
                                ...
                                className="px-2.5 py-1 bg-warn-soft hover:bg-warn text-white rounded text-[11px] font-bold transition shadow"
                              >
                                {isHi ? 'ROH' : 'ROH'}
                              </button>
                            )}
                            ...
                            <button
                              onClick={() => handleSelectComponent(comp)}
                              className="px-2.5 py-1 bg-raised hover:bg-selected text-ink-body rounded text-[11px] font-semibold transition"
                            >
        ...
        {selectedComponent && (
          <div className="bg-card border border-accent-line rounded-control p-5 space-y-5 flex flex-col h-fit">
            {/* Passport Header */}
            <div className="flex items-start justify-between border-b border-line pb-4">
              <div>
                <div className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 rounded-full bg-accent animate-pulse"></span>
                  <span className="text-[11px] font-bold uppercase text-accent-ink tracking-wider">{isHi ? 'डिजिटल हेल्थ पासपोर्ट' : 'Digital Health Passport'}</span>
                </div>
                <h3 className="text-lg font-extrabold text-white font-mono mt-1">
                  {selectedComponent.serialNumber}
                </h3>
                <p className="text-xs text-ink-body font-medium">{selectedComponent.partName}</p>
              </div>
              <button
                onClick={() => setSelectedComponent(null)}
                className="w-7 h-7 rounded-control bg-raised hover:bg-selected text-ink-muted hover:text-white flex items-center justify-center font-bold text-xs"
              >
                ✕
              </button>
            </div>
            ...
            {/* Health Score Gauge */}
            <div className="p-3.5 bg-page border border-line rounded-control space-y-2">
              <div className="flex justify-between items-center text-xs">
                <span className="text-ink-muted font-semibold">Degradation & Health Index:</span>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="p-2.5 bg-page rounded-control border border-line">
                <span className="text-[10px] text-ink-faint font-bold uppercase block">{isHi ? 'स्थिति' : 'Status'}</span>
                {getStatusBadge(selectedComponent.status)}
              </div>

              <div className="p-2.5 bg-page rounded-control border border-line">
                <span className="text-[10px] text-ink-faint font-bold uppercase block">{isHi ? 'वर्तमान स्थान' : 'Current Location'}</span>
              ...
              <div className="p-2.5 bg-page rounded-control border border-line">
                <span className="text-[10px] text-ink-faint font-bold uppercase block">{isHi ? 'निर्माता' : 'Manufacturer'}</span>
                <span className="font-medium text-ink-body truncate block">
                  {selectedComponent.manufacturer}
                </span>
              </div>

              <div className="p-2.5 bg-page rounded-control border border-line">
                <span className="text-[10px] text-ink-faint font-bold uppercase block">{isHi ? 'निर्माण तिथि' : 'Mfg Date'}</span>
                <span className="font-medium text-ink-body font-mono">
                  {selectedComponent.manufacturingDate}
                </span>
              </div>

              <div className="p-2.5 bg-page rounded-control border border-line">
                <span className="text-[10px] text-ink-faint font-bold uppercase block">{isHi ? 'कुल ओडोमीटर' : 'Total Odometer'}</span>
                <span className="font-bold text-accent-ink font-mono">
                  {selectedComponent.totalKmTravelled.toLocaleString()} km
                </span>
              </div>

              <div className="p-2.5 bg-page rounded-control border border-line">
                <span className="text-[10px] text-ink-faint font-bold uppercase block">{isHi ? 'पीओएच ओवरहॉल' : 'POH Overhauls'}</span>
                <span className="font-bold text-accent-ink font-mono">
            ...
            <div className="p-2.5 bg-page rounded-control border border-line font-mono text-[11px] break-all text-accent-ink">
              <span className="text-[10px] text-ink-faint uppercase block font-sans font-bold">QR Protocol URI:</span>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-2 bg-raised hover:bg-selected text-ink-body text-xs font-bold rounded-control border border-line transition"
              >
                <GearIcon size={15} className="inline align-[-2px] mr-1.5" />Adjust Health
              </button>
              ...
              {selectedComponent.status === 'AVAILABLE_IN_STORES' || selectedComponent.status === 'RECONDITIONED' ? (
                <button
                  ...
                  className="flex-1 py-2 bg-accent hover:bg-accent-hover text-white text-xs font-bold rounded-control transition shadow"
                >
                  Mount to Wagon
                </button>
                ...
                <button
                  ...
                  className="flex-1 py-2 bg-warn hover:bg-warn text-white text-xs font-bold rounded-control transition shadow"
                >{isHi ? 'हटाएँ' : 'Unassign'}</button>
              ) : null}
            </div>
            ...
            <div className="space-y-2.5 pt-3 border-t border-line">
              <h4 className="text-xs font-bold text-white uppercase tracking-[0.07em] flex items-center justify-between">
                <span className="inline-flex items-center gap-2"><FileTextIcon size={15} />Multi-Wagon Provenance Ledger</span>
                <span className="text-[10px] text-ink-faint">{isHi ? 'अपरिवर्तनीय ऑडिट ट्रेल' : 'Immutable Audit Trail'}</span>
              </h4>

              {historyLoading ? (
                <div className="p-4 text-center text-ink-faint text-xs font-mono">Loading history...</div>
              ) : componentHistory.length === 0 ? (
                <div className="p-3 text-center text-ink-faint text-xs bg-page rounded-control">
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                  {componentHistory.map((ev, idx) => (
                    <div
                      key={ev.id || idx}
                      className="p-2.5 bg-page border border-line rounded-control text-xs space-y-1"
                    >
                      <div className="flex justify-between items-center text-[10px]">
                        <span className="font-bold text-accent-ink">{ev.eventType}</span>
                        <span className="text-ink-faint font-mono">
                          {new Date(ev.createdAt).toLocaleString()}
                        </span>
                      </div>
                      <p className="text-ink-body text-[11px] leading-relaxed">{ev.actionDetails}</p>
                      <div className="flex justify-between items-center text-[10px] text-ink-muted">
                        {ev.wagonNumber && (
                          <span className="text-accent-ink font-mono">Wagon: {ev.wagonNumber}</span>
                        )}
                        <span>By: {ev.performerName}</span>
                      </div>
                      {ev.notes && (
                        <p className="text-[10px] text-ink-muted italic">Notes: {ev.notes}</p>
                      )}
                    </div>
                  ))}
                </div>
      ...
      {isRegisterModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-page backdrop-blur-sm p-4">
          <div className="bg-card border border-accent-line rounded-card p-6 max-w-lg w-full space-y-4">
            ...
            <form onSubmit={handleRegisterSubmit} className="space-y-3 text-xs">
              <div>
                <label className="block text-ink-body font-semibold mb-1">Serial Number *</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-ink-body font-semibold mb-1">{isHi ? 'घटक प्रकार' : 'Component Type'}</label>
                  <select
                ...
                <div>
                  <label className="block text-ink-body font-semibold mb-1">{isHi ? 'निर्माता' : 'Manufacturer'}</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-ink-body font-semibold mb-1">{isHi ? 'निर्माण तिथि' : 'Manufacturing Date'}</label>
                  <input
                    type="date"
                ...
                <div>
                  <label className="block text-ink-body font-semibold mb-1">{isHi ? 'बिन स्थान' : 'Bin Location'}</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-ink-body font-semibold mb-1">{isHi ? 'आरएफआईडी टैग (वैकल्पिक)' : 'RFID Tag (Optional)'}</label>
                <input
                  type="text"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-page backdrop-blur-sm p-4">
          <div className="bg-card border border-accent-line rounded-card p-6 max-w-md w-full space-y-4">
            <div className="flex justify-between items-center border-b border-line pb-3">
              ...
              <button onClick={() => setAssigningComponent(null)} className="text-ink-muted hover:text-white font-bold">
                ✕
              </button>
            </div>
            ...
            <form onSubmit={handleAssignSubmit} className="space-y-3 text-xs">
              <div>
                <label className="block text-ink-body font-semibold mb-1">Target Wagon Number *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-ink-body font-semibold mb-1">{isHi ? 'बोगी स्थान' : 'Bogie Position'}</label>
                <select
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-page backdrop-blur-sm p-4">
          <div className="bg-card border border-warn-line rounded-card p-6 max-w-md w-full space-y-4">
            <div className="flex justify-between items-center border-b border-line pb-3">
              ...
              <button onClick={() => setUnassigningComponent(null)} className="text-ink-muted hover:text-white font-bold">
                ✕
              </button>
            </div>
            ...
            <form onSubmit={handleUnassignSubmit} className="space-y-3 text-xs">
              <div>
                <label className="block text-ink-body font-semibold mb-1">{isHi ? 'लक्ष्य स्थिति' : 'Target Status'}</label>
                <select
              ...
              <div>
                <label className="block text-ink-body font-semibold mb-1">{isHi ? 'निरीक्षण टिप्पणी' : 'Inspection Notes'}</label>
                <textarea
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-line">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-warn hover:bg-warn text-white rounded-control font-bold shadow"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-page backdrop-blur-sm p-4">
          <div className="bg-card border border-accent-line rounded-card p-6 max-w-md w-full space-y-4">
            <div className="flex justify-between items-center border-b border-line pb-3">
              ...
              <button onClick={() => setHealthUpdateComponent(null)} className="text-ink-muted hover:text-white font-bold">
                ✕
              </button>
            </div>
            ...
            <form onSubmit={handleHealthSubmit} className="space-y-3 text-xs">
              <div>
                <label className="block text-ink-body font-semibold mb-1">
                  Health Score (0 - 100%): <span className="text-accent-ink font-bold">{healthForm.healthScore}%</span>
                </label>
                <input
                  type="range"
                  min={0}
              ...
              <div>
                <label className="block text-ink-body font-semibold mb-1">{isHi ? 'मूल्यांकन कारण' : 'Evaluation Justification'}</label>
                <textarea
