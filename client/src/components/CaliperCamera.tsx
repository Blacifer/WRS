  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 sm:p-5 shadow-lg space-y-4">
      ...
      <div className="flex justify-between items-center">
        ...
        {onClose && (
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white flex items-center justify-center font-bold text-sm">✕</button>
        )}
      </div>
      ...
      {activeMode === 'camera' && (
        <div className="space-y-4">
          ...
          <div className="relative aspect-[16/9] w-full bg-black rounded-xl overflow-hidden border-2 border-slate-700">
            {capturedPreviewUrl ? (
              <img
              ...
              <>
                ...
                <div className="absolute inset-x-8 sm:inset-x-20 inset-y-6 sm:inset-y-10 border-2 border-dashed border-blue-400/70 rounded-lg pointer-events-none flex items-center justify-center">
                  <span className="text-[11px] font-bold text-blue-300 bg-black/70 px-2 py-0.5 rounded tracking-wide">
            ...
            {isProcessing && (
              <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center gap-3">
                <RefreshCwIcon size={32} className="text-blue-400 animate-spin" />
            ...
            {capturedPreviewUrl ? (
              <>
                <button
                  ...
                  className="min-h-[52px] px-4 py-3 bg-slate-800 hover:bg-slate-700 active:bg-slate-900 text-slate-200 font-bold rounded-xl flex items-center justify-center gap-2 border border-slate-700 transition-all active:scale-[0.98]"
                >
              ...
              <>
                <button
                  ...
                  className="min-h-[52px] px-4 py-3 bg-blue-600 hover:bg-blue-500 active:bg-blue-700 text-white font-extrabold rounded-xl flex items-center justify-center gap-2 shadow-md transition-all active:scale-[0.98] disabled:opacity-40"
                >
                  <CameraIcon size={20} />
          ...
          {/* OCR Result or Error */}
          {ocrError && (
            <div className="p-3 bg-amber-950/50 border border-amber-500/30 rounded-xl flex items-start gap-2">
              <AlertTriangleIcon size={18} className="text-amber-400 shrink-0 mt-0.5" />
              <div>
                <p className="text-sm font-bold text-amber-300">{ocrError}</p>
                {ocrRawText && (
                ...
                <div className="mt-3 space-y-2">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 min-h-[44px] px-3 py-2 bg-slate-950 text-white font-mono text-lg font-bold rounded-lg border border-slate-700 outline-none"
                    />
                    <span className="text-slate-400 font-bold text-sm self-center">mm</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      {activeMode === 'manual' && (
        <div className="space-y-3">
          ...
          <div className="relative">
            ...
            />
            <span className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-sm">mm</span>
          </div>
          {manualError && (
